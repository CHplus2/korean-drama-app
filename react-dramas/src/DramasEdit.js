...
function DramasEdit({ setDramas }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await axios.put(`/api/dramas/${id}/`, payload, {
                withCredentials: true,
                headers: { "X-CSRFToken": getCookie("csrftoken") },
            });

            setDramas(prev => prev.map((d) => d.id === formData.id ? res.data : d));
            navigate("/");
        } catch (err) {
            console.error(err);
        }
